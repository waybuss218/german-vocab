...
async function loadCloudState(){const uid=userId();if(!uid)return;try{const {data,error}=await SB.from('user_state').select('state').eq('user_id',uid).maybeSingle();if(error)throw error;if(data?.state){state=migrateState(data.state)}else{state=blank()}await saveCloud(true)}catch(e){console.warn('Cloud state load failed:',e);showToast('云端同步暂时失败，仍保留本机记录。')}}rror_reports||{}),...(local.error_reports||{})}};state.history=dedupeById(state.history);state.unknown=dedupeUnknown(state.unknown);state.module_work=state.module_work||{}}await saveCloud(true)}catch(e){console.warn('Cloud state load failed:',e);showToast('云端同步暂时失败，仍保留本机记录。')}}
