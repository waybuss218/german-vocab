...
function renderHome(){const learned=new Set(state.history.map(x=>x.item_id)).size;const total=(DB.entries||[]).length;layout('','11 个部分 · 每部分 5 个模块 · 全部开放','home',`<section class="hero"><div><div class="kicker">VOCABULARY REVIEW</div><h1>按原书顺序，把词汇真正掌握。</h1><p>11 个部分全部开放；每个部分的 5 个模块也全部开放，不设置解锁顺序。</p></div><div class="stats"><div><strong>${learned} / ${total}</strong><span>已过单词</span></div><div><strong>${state.unknown.length}</strong><span>首次未知</span></div></div></section><section class="path">${DB.stages.map((s,i)=>stageCard(s,i+1)).join('')}</section>`)}
...
function showErrorTable(title){
 ...
 $('#startTargeted').onclick=()=>{state.module_work[k]={...(state.module_work[k]||{}),phase:'reinforce',round:2,items:[...pending],idx:0,show_table:false,errorHistoryIds:(state.module_work[k]?.errorHistoryIds||[]),updated_at:new Date().toISOString()};save();session.phase='reinforce';session.round=2;session.items=[...pending].map(id=>session.all.find(e=>e.id===id)).filter(Boolean);session.idx=0;session.errors=[];session.selected=null;renderQuestion()};
